...
export default function UserForm() {
    ...
    const onSubmit = (ev) => {
        ...
        setErrors(null);
        if(user.id) {
            api.put(`/users/${user.id}`, user).then(({data}) => {
                navigate("/users");
            }).catch(error => {
    ...
    return <div className="login-signup-form animated fadeInDown">
        <div className="form">
            ...
            {!loading && <form onSubmit={ev => onSubmit(ev)}>
                    ...
                    <input value={user.password} onChange={ev => setUser({...user, password: ev.target.value})} type="password" placeholder="Password" />
                    <input value={user.password_confirmation} onChange={ev => setUser({...user, password_confirmation: ev.target.value})} type="password" placeholder="Password Confirmation" />
                    <button className="btn block">Save</button>
                    <Link to="/users" className="btn__default">Cancel</Link>
                </form>
            }
        </div>
    </div>
